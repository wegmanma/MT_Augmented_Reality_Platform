...
static int cp2112_read(struct cp2112_device *dev, u8 *data, size_t size)
{
	...
	int ret;
	// uint64_t start_time = ktime_get_ns()/1000;
	// printk(KERN_INFO "CP2112: Reading! Start time = %lld\n", start_time);
	...
	// printk(KERN_INFO "CP2112: Atomic Set! %lld\n",ktime_get_ns()/1000-start_time);
	...
	// printk(KERN_INFO "CP2112: HID_warn! %lld\n",ktime_get_ns()/1000-start_time);
	ret = cp2112_wait(dev, &dev->read_avail);
	if (ret)
		return ret;
	// printk(KERN_INFO "CP2112: wait...?! %lld\n",ktime_get_ns()/1000-start_time);
	hid_dbg(hdev, "read %d of %zd bytes requested\n",
		dev->read_length, size);

	if (size > dev->read_length)
		size = dev->read_length;
	// printk(KERN_INFO "CP2112: HID_dbg! %lld\n",ktime_get_ns()/1000-start_time);
	memcpy(data, dev->read_data, size);
	// printk(KERN_INFO "CP2112: memcpy! %lld\n",ktime_get_ns()/1000-start_time);
...
static int cp2112_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	printk(KERN_INFO "Before: %d\n", be32_to_cpu(config.clock_speed));
	config.retry_time = cpu_to_be16(1);
	config.clock_speed = cpu_to_be32(1000000);
	printk(KERN_INFO "After: %d\n", be32_to_cpu(config.clock_speed));
	printk(KERN_INFO "Timeout: %d\n", be16_to_cpu(config.read_timeout));
